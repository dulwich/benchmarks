...
class LargeHistoryBenchmarks(BenchmarkBase):
    ...
    def time_merge_base(self, num_commits, files_per_commit):
        ...
        if num_commits >= 200:
            ...
            branch2 = self.repo.refs[f"refs/heads/branch-{(num_commits // 100) * 100}".encode()]
    ...
    def setup(self, num_objects, with_deltas):
        ...
        for i in range(num_objects):
            if with_deltas and i > 0 and i % 10 != 0:
                ...
                content = base_content[:9000] + f"\nModification {i}\n" + base_content[9000:]
            else:
                # Create unique content
                content = ''.join(random.choices(string.ascii_letters + string.digits, k=10000))
                if with_deltas:
...
class PackNegotiationBenchmarks(BenchmarkBase):
    ...
    def time_negotiate_pack(self, num_commits, common_history_percent):
        ...
        haves = [sha for sha in self.common_shas if sha in self.client_repo.object_store]
...
class BranchOperationBenchmarks(BenchmarkBase):
    ...
    def setup(self, num_branches):
        ...
        for i in range(num_branches):
            ...
            commit = self.repo.do_commit(
                f"Branch {i} commit".encode(),
                parent_commits=[base_commit]
            )
    ...
    def time_merge_branches(self, num_branches):
        ...
        if num_branches >= 2:
            ...
            merge_commit.author = merge_commit.committer = b"Test User <test@example.com>"
